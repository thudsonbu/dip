...
# directory "root/d1/d2/.../dm". Note that n >= 1 and m >= 0. If m = 0, it means
...
# path is a string that has the following format: "directory_path/file_name.txt"
...
class Solution:
    def findDuplicate(self, paths):
        ...
        for path in paths:
            self.recordPath(path, dups, seen)

        return [seen[dup] for dup in dups]

    def recordPath(self, path, dups, seen):
